...
add_action('admin_menu', 'db_admin_page_menu');
add_action('admin_bar_menu', 'db_bar_menu',1000);
 

function db_bar_menu() {
    global $wp_admin_bar, $wpdb;
	$wp_admin_bar->add_menu( array( 'id' => 'qp_bar_menu', 'title' => __( 'Test Menu', 'textdomain' ), 'href' => FALSE ) );
	...
	{
	//$sel='edit.php?post_type=page';
	//$submenu[$sel][] = array($page->post_title, 'manage_options', 'post.php?post='.$page->ID.'&action=edit');
	$menu_title=$page->post_title;
	$menu_url='post.php?post='.$page->ID.'&action=edit';
	//add_submenu_page( 'qpages', '', $menu_title, 'manage_options', $menu_url);
	$wp_admin_bar->add_menu( array( 'parent' => 'qp_bar_menu', 'title' =>$menu_title, 'href' => $menu_url ) );
	}
...
function db_admin_page_menu() {
	...
	//$submenu[]=array('google','manage_option','http://google.com');
	$querystr = "SELECT $wpdb->posts.ID,$wpdb->posts.post_title FROM $wpdb->posts WHERE $wpdb->posts.post_status = 'publish' AND $wpdb->posts.post_type = 'page' AND $wpdb->posts.post_parent =0 ORDER BY $wpdb->posts.menu_order ASC";
